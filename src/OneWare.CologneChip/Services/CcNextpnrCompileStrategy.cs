...
public class CcNextpnrCompileStrategy : ICologneChipCompileStrategy
{
    ...
    public CcNextpnrCompileStrategy()
    {
        ...
        try
        {
            ...
            switch (topLanguage)
            {
                ...
                case "vhd":
                    ...
                    yosysArguments = ["-q","-l","./synth.log",  "-p", $"ghdl --warn-no-binding -C --ieee=synopsys ./../{top} -e {topName}; {yosysSynthTool} -nomx8 -top {topName} -luttree -nomult; write_json {topName}.json;"];
                    includedExtensions = [];
                    break;
                case "v": 
                    _outputService.WriteLine("Verilog Synthesis...\n==============");
                    yosysArguments = ["-p", $"synth_gatemate -nomx8 -top {topName} -luttree -nomult; write_json {topName}.json;"];
    ...
    public async Task<bool> PrAsync(UniversalFpgaProjectRoot project, FpgaModel fpgaModel)
    {
        ...
        List<string> prArguments = ["--device=CCGM1A1","--json", $"{topName}.json", "-o", $"ccf=./../{ccfFile}", "-o", $"out={topName}_impl.txt", "--router=router2"];
        
        var success = (await _childProcessService.ExecuteShellAsync("nextpnr-himbaechel", prArguments,
    ...
    public async Task<bool> PackAsync(UniversalFpgaProjectRoot project, FpgaModel fpgaModel)
    {
        ...
        List<string> prArguments = [$"{topName}_impl.txt", $"{topName}.bit"];
        
        var success = (await _childProcessService.ExecuteShellAsync("gmpack", prArguments,
            $"{project.FullPath}/build", $"Running P_R...", AppState.Loading, true, null, s =>
            {
                Dispatcher.UIThread.Post(() => { _outputService.WriteLine(s); });
                return true;
